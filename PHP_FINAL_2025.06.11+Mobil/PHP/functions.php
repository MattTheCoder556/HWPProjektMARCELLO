...
function redirectToRegister($message, $formData = [])
{
	...
	header("Location: https://mmm.stud.vts.su.ac.rs/PHP/register.php");
	exit();
}
function redirectToLogin($message, $formData = [], $logged)
{
    ...
    if($logged == 1)
    {
        header("Location: https://mmm.stud.vts.su.ac.rs/PHP/login.php");
        exit();
    }
    else
    {
...
function sendVerificationEmail($email, $token): void
{
	...
	try
	{
		...
		$verificationLink = "https://mmm.stud.vts.su.ac.rs/PHP/verifyUser.php?token=$token";
...
function send_password_reset_email($email, $token)
{
	...
	try
	{
		...
		*/
		...
		$resetLink = "https://mmm.stud.vts.su.ac.rs/PHP/reset_password.php?token=$token";
		$mail->isHTML(true);
...
function sendInviteEmail($email, $inviteToken, $inviter, $wishlistHtml, $templateData): void
{
    ...
    try {
        ...
        // Invitation links
        $baseUrl = "https://mmm.stud.vts.su.ac.rs/PHP/logged_in_sites/";
